defmodule GoGameWeb.Router do
  ...
  scope "/api", GoGameWeb do
    pipe_through :api

    get "/cluster-status", PageController, :cluster_status
  end
  ...
  # --- 4. ADMIN ROUTES (Admins Only) ---
  scope "/admin", GoGameWeb do
    pipe_through [:browser, :require_authenticated_user]

    live_session :require_admin,
      on_mount: [{GoGameWeb.UserAuth, :require_admin}] do
      live "/users", AdminLive.Users, :index
    end
  end

  # --- 5. PUBLIC AUTH ROUTES (Register, Login) ---
  scope "/", GoGameWeb do
    pipe_through [:browser]
