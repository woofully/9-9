defmodule GoGameWeb.GameLive.Show do
  ...
  def mount(%{"id" => game_id}, _session, socket) do
    ...
    {:ok,
     assign(socket,
       ...
       black_player_name: if(String.starts_with?(db_game.black_player.username, "Guest_"), do: "游客", else: db_game.black_player.username),
       white_player_name: if(String.starts_with?(db_game.white_player.username, "Guest_"), do: "游客", else: db_game.white_player.username),
       game_id: game_id
     )}
  end
      ...
      <div class="flex flex-col items-center w-full px-2">
        ...
        <div class="w-full max-w-2xl flex justify-between items-center mb-4 font-serif text-[#5e4b35]">
          ...
          <div class={"flex flex-col items-center p-2 rounded-lg border-2 transition-all duration-300 " <>
            ...
            <div class="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-gray-700 to-black shadow-md mb-1"></div>
          ...
          <div class={"flex flex-col items-center p-2 rounded-lg border-2 transition-all duration-300 " <>
            ...
            <div class="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-white to-gray-200 shadow-md border border-gray-300 mb-1"></div>
        ...
        <div class="mb-8 w-full max-w-2xl">
          <BoardRenderer.board board={@game.board} interactive={@db_game.status == "in_progress" && !@game.game_over} />
        </div>
          ...
          <div class="w-full max-w-2xl mb-4 bg-[#dcb35c] border-2 border-[#cda24b] rounded-lg p-4 shadow-lg">
            <div class="text-center mb-3">
              <p class="text-lg font-serif font-semibold text-[#5e4b35]">
                <%= if @game.count_requested_by == :black, do: "黑棋", else: "白棋" %>要求点目，是否接受？
              </p>
        ...
        <%= if @db_game.status == "pending" do %>
          ...
          <div class="w-full max-w-2xl mb-4">
            <div class="bg-white rounded-lg border-2 border-[#d8cbb3] p-6 shadow-lg text-center">
              <p class="text-lg font-serif text-[#5e4b35] mb-2">等待对方加入游戏...</p>
              <.link navigate={~p"/lobby"} class="inline-block px-6 py-2 text-base font-serif text-[#5e4b35] bg-[#e8e0c5] hover:bg-[#d8cbb3] rounded transition-all">
                返回大厅
              </.link>
            </div>
          </div>
          ...
          <div class="mb-4 w-full max-w-2xl">
            <%= if @game.score do %>
              ...
              <div class="bg-white rounded-lg border-2 border-[#d8cbb3] p-6 mb-4 shadow-lg">
                <h3 class="text-xl font-serif font-bold text-[#5e4b35] text-center mb-4">点目结果</h3>
                <div class="flex justify-around mb-4">
                  <!-- Black Score -->
                  <div class="text-center">
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-gray-700 to-black shadow-md mx-auto mb-2"></div>
                    <div class="font-serif text-[#5e4b35] font-semibold mb-2">黑棋</div>
                    <div class="text-sm font-serif text-gray-600">
                      <div>棋子: <%= @game.score.black.stones %></div>
                      <div>地域: <%= @game.score.black.territory %></div>
                      <div class="border-t border-gray-300 mt-1 pt-1 font-bold">总计: <%= @game.score.black.total %></div>
                    </div>
                  </div>

                  <!-- White Score -->
                  <div class="text-center">
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-white to-gray-200 shadow-md border border-gray-300 mx-auto mb-2"></div>
                    <div class="font-serif text-[#5e4b35] font-semibold mb-2">白棋</div>
                    <div class="text-sm font-serif text-gray-600">
                      <div>棋子: <%= @game.score.white.stones %></div>
                      <div>地域: <%= @game.score.white.territory %></div>
                      <div>贴目: <%= @game.score.white.komi %></div>
                      <div class="border-t border-gray-300 mt-1 pt-1 font-bold">总计: <%= @game.score.white.total %></div>
                    </div>
                  </div>
                </div>
                ...
                <div class="text-center text-2xl font-serif font-bold text-[#5e4b35] py-3 bg-[#f3ead7] rounded">
                  <%= if @game.winner == :black, do: "黑棋", else: "白棋" %> 获胜！
                </div>
              </div>
            <% else %>
              <!-- Resign - No Score -->
            ...
            <div class="text-center">
              <.link navigate={~p"/lobby"} class="inline-block px-8 py-3 text-xl font-serif text-white bg-[#5e4b35] hover:bg-[#4a3a28] rounded shadow-md transition-all">
                返回大厅
              </.link>
            </div>
          </div>
        ...
        <div class="text-xs text-gray-400 font-mono text-center">
          当前回合: <%= @game.turn %> | 历史记录: <%= length(@game.history) %> 步
        </div>
      </div>
    </div>
    """
  ...
  def handle_info(%Phoenix.Socket.Broadcast{event: "game_updated", payload: payload}, socket) do
    ...
    updated_socket = if Map.has_key?(payload, :game) do
      updated_socket
      |> clear_flash()
  ...
  def handle_info(:tick, socket) do
    ...
    if game.game_over do
      ...
      updated_game = case game.turn do
