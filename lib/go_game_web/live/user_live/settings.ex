defmodule GoGameWeb.UserLive.Settings do
    ...
    <div class="min-h-screen bg-[#f3ead7] py-8 px-4">
      <div class="mx-auto max-w-2xl">
        <div class="bg-white rounded-lg shadow-xl border-2 border-[#d8cbb3] p-8">
          <!-- Back to Lobby Link -->
          <div class="mb-4">
            <.link navigate={~p"/lobby"} class="inline-flex items-center text-[#5e4b35] hover:text-[#4a3a28] font-serif transition-colors">
          ...
          <h1 class="text-3xl font-serif font-bold text-[#5e4b35] mb-2 text-center">账户设置</h1>
          <p class="text-center text-[#8b7355] mb-8 font-serif">管理您的用户名和密码</p>

          <div class="space-y-8">
            <!-- Username Section -->
            <div class="pb-8 border-b border-[#e8e0c5]">
              <h2 class="text-xl font-serif font-semibold text-[#5e4b35] mb-4">修改用户名</h2>
              <.simple_form
                for={@username_form}
                id="username_form"
              ...
              >
                <.input field={@username_form[:username]} type="text" label="用户名" required />
                <.input
                ...
                />
                <:actions>
                  <.button class="w-full bg-[#5e4b35] hover:bg-[#4a3a28] text-white font-serif py-3 rounded-lg transition-colors" phx-disable-with="修改中...">
                    修改用户名
                  </.button>
                </:actions>
              </.simple_form>
            ...
            <!-- Password Section -->
            <div>
              <h2 class="text-xl font-serif font-semibold text-[#5e4b35] mb-4">修改密码</h2>
              <.simple_form
                for={@password_form}
                id="password_form"
  ...
  def handle_event("update_username", params, socket) do
    ...
    case Accounts.update_user_username(user, password, user_params) do
      {:ok, updated_user} ->
        info = "用户名修改成功。"
        {:noreply, socket |> put_flash(:info, info) |> assign(username_form_current_password: nil, current_username: updated_user.username)}
