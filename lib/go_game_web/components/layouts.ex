    ...
    <div class="relative" id="user-menu" phx-click-away={JS.hide(to: "#user-menu-dropdown")}>
      ...
      >
        ...
        <span class="font-serif text-[#5e4b35] font-semibold">
          <%= if @current_user.is_guest, do: "游客", else: @current_user.username %>
        </span>
        <!-- Dropdown Arrow -->
      ...
      >
        <div class="py-2">
          <%= if @current_user.is_guest do %>
            <!-- Guest User Options -->
            <a href="/users/register" class="block px-4 py-2 text-[#5e4b35] hover:bg-[#f3ead7] font-serif transition-colors">
              设置用户名和密码
            </a>
          <% else %>
            <!-- Registered User Options -->
            <a href="/users/settings" class="block px-4 py-2 text-[#5e4b35] hover:bg-[#f3ead7] font-serif transition-colors">
              设置
            </a>

            <%= if @current_user.is_admin do %>
              <!-- Admin Link -->
              <a href="/admin/users" class="block px-4 py-2 text-purple-700 hover:bg-purple-50 font-serif transition-colors border-t border-[#e8e0c5]">
                👑 管理面板
              </a>
            <% end %>
          <% end %>
          ...
          <form action="/users/log-out" method="post" class="m-0">
            ...
            <button type="submit" class="w-full text-left px-4 py-2 text-[#5e4b35] hover:bg-[#f3ead7] font-serif transition-colors border-t border-[#e8e0c5]">
              退出登录
            </button>
          </form>
        </div>
