defmodule GoGame.Gameplay.Rules do
  ...
  defstruct board: %{},          # Map %{{x,y} => :black | :white}
            turn: :black,        # :black | :white
            captures: %{black: 0, white: 0},
            history: [],         # List of previous board states (for Ko checks)
            moves: [],           # List of moves [{:black, {x,y}}, ...] (for SGF)
            passed: false,       # Track if previous move was a pass (for game end)
            count_requested_by: nil, # :black | :white | nil - who requested to count points
            black_time: 600,     # Black player's time in seconds (10 minutes)
            white_time: 600,     # White player's time in seconds (10 minutes)
            turn_started_at: nil, # Unix timestamp when current turn started
            game_over: false,    # Track if game has ended
            winner: nil,         # :black | :white | nil
            score: nil           # Score details: %{black: %{stones: N, territory: N, total: N}, white: %{...}}
      ...
      if captured_count == 0 and count_liberties(board_after_captures, {x, y}) == 0 do
        ...
        case check_ko(board_after_captures, game.history) do
          ...
          :ok ->
            ...
            {black_time, white_time} = case game.turn do
            ...
            {:ok, %{game |
              ...
              history: [game.board | game.history], # Prepend OLD board to history
              moves: [{game.turn, {x, y}} | game.moves],
              passed: false,
              count_requested_by: nil, # Clear count request when move is made
  ...
  defp count_liberties(board, group) when is_list(group) do
    group
    |> Enum.flat_map(&get_neighbors/1) # Get all neighbors of all stones
    |> Enum.uniq()                     # Remove duplicates
    |> Enum.filter(fn pos -> not Map.has_key?(board, pos) end) # Keep only empty spots
    |> length()
  end
  defp count_liberties(board, single_stone), do: count_liberties(board, get_group(board, single_stone, Map.get(board, single_stone)))
  ...
  defp calculate_territory(board) do
    ...
    Enum.reduce(territories, {0, 0}, fn territory, {black_acc, white_acc} ->
      case determine_territory_owner(board, territory) do
        ...
        :neutral -> {black_acc, white_acc}  # Neutral territory (dame) counts for no one
      end
    end)
  end
  ...
  defp determine_territory_owner(board, territory) do
    ...
    case bordering_colors do
      [:black] -> :black
      [:white] -> :white
      _ -> :neutral  # Bordered by both colors or no stones
    end
  end
end
