defmodule GoGame.Accounts.User do
  ...
  def guest_changeset(attrs \\ %{}) do
    %__MODULE__{}
    |> cast(attrs, [:username, :location_city])
  ...
  defp validate_email(changeset, opts) do
    ...
    if allow_no_email do
      changeset
      |> validate_format(:email, ~r/^[^\s]+@[^\s]+$/, message: "必须包含@符号且不能有空格")
      |> validate_length(:email, max: 160)
      |> unsafe_validate_unique(:email, GoGame.Repo)
      |> unique_constraint(:email, message: "该邮箱已被使用")
    else
      changeset
  ...
  defp validate_username(changeset) do
    ...
    |> validate_format(:username, ~r/^[a-zA-Z0-9_]+$/, message: "只允许字母、数字和下划线")
  ...
  def email_changeset(user, attrs, opts \\ []) do
    ...
    |> case do
      ...
      %{} = changeset -> add_error(changeset, :email, "邮箱未发生变化")
    end
  end
  ...
  def username_changeset(user, attrs \\ %{}) do
    ...
    |> case do
      ...
      %{} = changeset -> add_error(changeset, :username, "用户名未发生变化")
    end
  end
  ...
  def password_changeset(user, attrs, opts \\ []) do
    user
    |> cast(attrs, [:password])
    |> validate_confirmation(:password, message: "两次输入的密码不一致")
    |> validate_password(opts)
  end
  ...
  def validate_current_password(changeset, password) do
    if valid_password?(changeset.data, password) do
      ...
      add_error(changeset, :current_password, "密码不正确")
    end
  end
end
